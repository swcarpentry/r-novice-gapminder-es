...
Primero modifiquemos la clase de la columna 'color' a factor utilizando la función `factor` y luego intentemos agregar una nueva fila a nuestro *data frame*:

```{r}
gatos$color <- factor(gatos$color)
nueva_fila <- list("tortoiseshell", 3.3, TRUE, 9)
gatos <- rbind(gatos, nueva_fila)
```
...
Esta base de datos reune información sobre la esperanza de vida, el PIB per cápita y la población por país colectada por la organización [Gapminder](http://www.gapminder.org/data/)
